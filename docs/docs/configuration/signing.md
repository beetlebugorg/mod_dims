...
the commands and the image URL, so neither can be changed without the secret.
...
The commands have a trailing slash in the message and the image URL is
percent encoded in the query string but not in the message.
...
`overlay` and `optimizeResize` are only covered when `_keys` lists them.
Without that, anyone holding a valid URL can change either.

:::warning
Six hexadecimal characters is 24 bits. One in 16.8 million random signatures is
accepted, so a caller sending requests at a moderate rate finds a working
